...
/** WebSocket relay path — must live under /api so the platform ingress routes it here. */
const WS_PATH = '/api/net';
...
function countPlayers(): number {
  let n = 0;
  for (const r of rooms.values()) n += (r.clients[0] ? 1 : 0) + (r.clients[1] ? 1 : 0);
  return n;
}
...
const httpServer = createServer((req: IncomingMessage, res: ServerResponse) => {
  ...
  if (req.method === 'GET' && url === '/api/version') {
    json(res, 200, { protocol: PROTOCOL_VERSION, service: 'mugen-netplay-relay' });
    return;
  }
  if (req.method === 'GET' && url === '/api/rooms') {
    json(res, 200, { rooms: rooms.size, players: countPlayers() });
    return;
  }
...
httpServer.listen(PORT, () => {
  console.log(`mugen relay: http :${PORT} (GET /health, /api/*) · ws ${WS_PATH}`);
});
