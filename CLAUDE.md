...
Browser 2D fighting game. Monorepo (npm workspaces):

- `shared/` — deterministic 60 Hz sim core + character data. Integers only
  (`256 units = 1 px`), seeded RNG lives in `SimState`, no `Math.random/sin/cos`
  in `shared/src/sim/step.ts`. `npm test` + `npm run verify:determinism` guard this.
- `frontend/` — Vite game client. Canvas renderer, WebAudio synth, rule-based CPU,
  delay-based lockstep netplay client. Generated sprite/stage art under
  `frontend/public/assets/` is committed (the deploy image runs no codegen);
  regenerate with `npm run gen:assets` and commit when sprites change.
- `backend/` — Node HTTP (`GET /health`, `/api/*`) + WebSocket lockstep relay on
  `:8000`. Pure relay: rooms by 6-char code, no simulation, no persistence.

`npm run dev` runs both. `npm run build` / `npm run typecheck` / `npm test`.
